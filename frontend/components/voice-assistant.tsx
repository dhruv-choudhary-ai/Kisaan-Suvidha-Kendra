    ...
    try {
      ...
      if (data.language_detected && data.language) {
        ...
        setTimeout(() => {
          ...
          // Auto-start recording after language detection confirmation
          // Add longer delay to ensure audio has finished and mic is available
          setTimeout(() => {
            console.log("[v0] Auto-starting recording after language detection")
            startRecording().catch(err => {
              console.error("[v0] Failed to auto-start recording:", err)
              // Don't retry automatically - let user click manually
              setStatus("active")
            })
  ...
  const handleLanguageSelection = async (languageName: string) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        // Auto-start recording after language confirmation
        // Add longer delay and better error handling
        setTimeout(() => {
          console.log("[v0] Auto-starting recording after manual language selection")
          startRecording().catch(err => {
            console.error("[v0] Failed to auto-start recording:", err)
            // Don't retry automatically - let user click manually
            setStatus("active")
                ...
                <div className="flex flex-col items-center gap-6 pt-6">
                  <div className="relative">
                    ...
                    {(status === "processing") ? (
                      ...
                      <Button
                        ...
                        className={`h-24 w-24 rounded-full transition-all duration-500 shadow-xl relative group border-4 border-white/50 ${
                          isRecording && status === "listening"
                            ? "bg-gradient-to-br from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white"
                            : "bg-gradient-to-br from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white hover:scale-110 hover:shadow-2xl"
                        }`}
                      >
                        {isRecording && status === "listening" ? (
                  ...
                  <div className="text-center space-y-2">
                    <p className="text-base font-semibold text-gray-800">
                      ...
                      {status === "active" && !isSpeaking && "Tap the microphone to speak"}
                      {status === "active" && isSpeaking && "Playing response..."}
                    </p>
                    <p className="text-sm text-gray-600">
                      {status === "active" && !isSpeaking && "Ask me anything about farming, crops, or weather"}
                      {status === "listening" && "Release to auto-send when you finish speaking"}
                      {isSpeaking && "Please wait for the response to finish"}
                    </p>
                  </div>
                </div>
              </>
